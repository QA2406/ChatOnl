...
public class MethodClient{
    ...
    private static Vector<JPanel> hsChat;
    ...
    public static void connect(ImageIcon icon, String userName, String password) throws Exception {
        client = new Socket("localhost", 24242);
        out = new ObjectOutputStream(client.getOutputStream());
        in = new ObjectInputStream(client.getInputStream());
        SimpleDateFormat df = new SimpleDateFormat("hh:mm aa");
        String t = df.format(new Date());
        Message ms = new Message("login",userName,myID,password ,"Server",null);
        out.writeObject(ms);
        out.flush();
        myName = userName;
        time = t;
    }
    public static void sendMessage(String text,String target,ImageIcon avatar) throws Exception {
    	Message ms = new Message("message", MethodClient.getMyName(),MethodClient.getMyID(), text,target, avatar);
        System.out.println(MethodClient.getMyID()+ "QQQQQ");
    	out.writeObject(ms);
        out.flush();
    }
  ...
  //      Message ms = new Message();
    ...
    public static void downloadFile(int ID, String name) {
        try {
            ...
            int c = ch.showSaveDialog(Messenger.getFrames()[0]);
            if (c == JFileChooser.APPROVE_OPTION) {
                File f = ch.getSelectedFile();
                if (f.exists()) {
                    int click = JOptionPane.showConfirmDialog(Messenger.getFrames()[0], "This file name has already do you want to replace", "Save File", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
    ...
    public static int get(int index) {
    	MethodClient.getFriends().get(index);
    	return index;
    }
    ...
    public static void setFrame(JFrame aFram) {
        fram = aFram;
    }
}